...
class Friction extends Component {
  ...
  handleClick = () => {
    this.props.mapContainer.setEnable('enableFriction');
  };

  render() {
    const { enableFriction } = this.props.mapContainer.state;

    return (
      <Menu vertical borderless size='huge'>
        <Menu.Item>
          <Checkbox toggle label='Enable' checked={enableFriction === true} onClick={this.handleClick} />
        </Menu.Item>
        <Menu.Item header>
          Friction
        </Menu.Item>
